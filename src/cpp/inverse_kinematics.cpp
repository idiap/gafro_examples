...
int main(int /*argc*/, char ** /*argv*/)
{
    ...
    gafro::Motor<double> ee_target_motor = panda.getEEMotor(panda.getRandomConfiguration());

    gafro::SingleManipulatorMotorCost<double, 7> cost_function(&panda, ee_target_motor);
    ...
    for (; i < 100; ++i)
    {
        ...
        Eigen::Vector<double, 7> update = -(hessian.inverse() * gradient);
    ...
    std::cout << "------------------------------------------------------" << std::endl;
    std::cout << "Target Motor:" << ee_target_motor << std::endl;
    std::cout << "Solution Motor:" << panda.getEEMotor(x) << std::endl;
}